...
function config(areas: Area[], options: any, drawing: Record<string, any>): Record<Area, Config> {
	...
	for(const area of areas)
	{
		...
		const colorSpace = _drawing.colorSpace ?? 'RGBA';
...
async function addWithConfig(options: any, {colorSpace, applyIn, config, modes}: Config, area: Area, isDegraded: boolean = false): Promise<any> {
	...
	if(applyIn !== 'without' && (isDegraded || applyIn === 'both'))
	{
		...
		{	
			await krita.send(`set_layer_color_space:${JSON.stringify({
				name: layerName,
